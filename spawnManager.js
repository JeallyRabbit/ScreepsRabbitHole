...
Room.prototype.spawnManager = function spawnManager() {
    ...
    if (global.heap.rooms[this.name].offensiveQueue.length > 0 && global.heap.rooms[this.name].offensiveQueue[0].role == C.ROLE_QUAD_MEMBER &&
        ...
        var result = spawn.spawnCreep(body, name + '_' + this.name + Game.time, { memory: { type: C.ROLE_QUAD_MEMBER, quadId: request.quadId, homeRoom: this.name } })
        if (result == OK) {
    ...
    if (global.heap.rooms[this.name].defensiveQueue.length > 0) {
        ...
        switch (role) {
            case C.ROLE_SOLDIER:
                {
                    var result = spawn.spawnCreep(soldierBody(energyCap, request.isMelee), C.ROLE_SOLDIER + '_' + this.name + Game.time, { memory: { type: C.ROLE_SOLDIER, homeRoom: this.name, targetRoom: request.roomName } })
                    global.heap.rooms[this.name].spawnResult = result
        ...
        switch (role) {
            case C.ROLE_HARVESTER:
                {
                    var result = spawn.spawnCreep(harvesterBody(energyCap), C.ROLE_HARVESTER + '_' + this.name + Game.time, { memory: { type: C.ROLE_HARVESTER, homeRoom: this.name, targetRoom: request.sourceRoom, sourceId: request.sourceId } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
            ...
            case C.ROLE_CARRIER:
                {
                    var result = spawn.spawnCreep(carrierBody(energyCap), C.ROLE_CARRIER + '_' + this.name + Game.time, { memory: { type: C.ROLE_CARRIER, homeRoom: this.name, targetRoom: request.sourceRoom, sourceId: request.sourceId } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].harvestingQueue.shift()
            ...
            case C.ROLE_FILLER:
                {
                    ...
                    var result = spawn.spawnCreep(body, C.ROLE_FILLER + '_' + this.name + Game.time, { memory: { type: C.ROLE_FILLER, homeRoom: this.name, spawnId: this.id } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].harvestingQueue.shift()
            ...
            case C.ROLE_HAULER:
                {
                    var result = spawn.spawnCreep(carrierBody(energyCap), C.ROLE_HAULER + '_' + this.name + Game.time, { memory: { type: C.ROLE_HAULER, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
        ...
        switch (role) {
            ...
            case C.ROLE_SCOUT:
                {
                    ...
                    var result = spawn.spawnCreep([MOVE], C.ROLE_SCOUT + '_' + this.name + Game.time, { memory: { type: C.ROLE_SCOUT, homeRoom: this.name, homeSpawnID: spawn.id, targetRoom: targetRoom } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_WORKER:
                {
                    ...
                    var result = spawn.spawnCreep(body, C.ROLE_WORKER + '_' + this.name + Game.time, { memory: { type: C.ROLE_WORKER, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_REPAIRER:
                {
                    var result = spawn.spawnCreep(repairerBody(energyCap), C.ROLE_REPAIRER + '_' + this.name + Game.time, { memory: { type: C.ROLE_REPAIRER, targetRoom: request.roomName, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_RESERVER:
                {
                    var result = spawn.spawnCreep([MOVE, CLAIM], C.ROLE_RESERVER + '_' + this.name + Game.time, { memory: { type: C.ROLE_RESERVER, homeRoom: this.name, targetRoom: request.roomName } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_RAMPART_REPAIRER:
                {
                    var result = spawn.spawnCreep(workerBody(energyCap), C.ROLE_RAMPART_REPAIRER + '_' + this.name + Game.time, { memory: { type: C.ROLE_RAMPART_REPAIRER, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_RESOURCE_MANAGER:
                {
                    var result = spawn.spawnCreep(carrierBody(energyCap), C.ROLE_RESOURCE_MANAGER + '_' + this.name + Game.time, { memory: { type: C.ROLE_RESOURCE_MANAGER, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_CLAIMER:
                {
                    var result = spawn.spawnCreep([MOVE, MOVE, CLAIM], C.ROLE_CLAIMER + '_' + this.name + Game.time, { memory: { type: C.ROLE_CLAIMER, homeRoom: this.name, targetRoom: request.roomName } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_COLONIZER:
                {
                    var result = spawn.spawnCreep(workerBody(energyCap, [MOVE, CARRY, WORK, MOVE]), C.ROLE_COLONIZER + '_' + this.name + Game.time, { memory: { type: C.ROLE_COLONIZER, homeRoom: this.name, targetRoom: request.roomName } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_MINER:
                {
                    var result = spawn.spawnCreep(minerBody(energyCap), C.ROLE_MINER + '_' + this.name + Game.time, { memory: { type: C.ROLE_MINER, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_MINERAL_CARRIER:
                {
                    var result = spawn.spawnCreep(carrierBody(energyCap), C.ROLE_MINERAL_CARRIER + '_' + this.name + Game.time, { memory: { type: C.ROLE_MINERAL_CARRIER, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_DOCTOR:
                {
                    var result = spawn.spawnCreep(carrierBody(energyCap), C.ROLE_DOCTOR + '_' + this.name + Game.time, { memory: { type: C.ROLE_DOCTOR, homeRoom: this.name } })
                    global.heap.rooms[this.name].spawnResult = result
                    global.heap.rooms[this.name].spawnRole = role
                    if (result == OK) {
                        global.heap.rooms[this.name].civilianQueue.shift()
            ...
            case C.ROLE_ENERGY_DRAINER:
                {
                    if (this.storage[RESOURCE_ENERGY] > C.STORAGE_ENERGY_BOTTOM) {
                        ...
                        var result = spawn.spawnCreep(body, name + '_' + this.name + Game.time, { memory: { type: C.ROLE_ENERGY_DRAINER, targetRoom: request.roomName, homeRoom: this.name } })
                        global.heap.rooms[this.name].spawnResult = result
                        global.heap.rooms[this.name].spawnRole = role
                        if (result == OK) {
                            global.heap.rooms[this.name].offensiveQueue.shift()
